...
const s = StyleSheet.create({
  ...
  notesTitle: { fontSize: 9, fontFamily: 'Helvetica-Bold', marginBottom: 4, color: BLUE },
      ...
      <Page size="A4" style={s.page}>
        ...
        <View style={s.header}>
          ...
          <View>
            ...
            <View style={s.metaRow}>
              <Text style={s.metaLabel}>Créé le</Text>
              <Text style={s.metaValue}>{new Date(data.dateCreation).toLocaleDateString('fr-FR')}</Text>
            </View>
            <View style={s.metaRow}>
              <Text style={s.metaLabel}>Valide jusqu'au</Text>
              <Text style={s.metaValue}>{new Date(data.dateValidite).toLocaleDateString('fr-FR')}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={s.tableHead}>
          ...
          <Text style={[s.thCell, s.cRemise]}>Rem.</Text>
        ...
        {data.lignes.map((l, i) => (
          <View key={i} style={i % 2 !== 0 ? [s.tableRow, s.tableRowAlt] : s.tableRow}>
            ...
            <Text style={[s.cRemise, { color: l.remise > 0 ? ORANGE : GRAY }]}>
              {l.remise > 0 ? `-${l.remise}%` : '—'}
            </Text>
        ...
        <View style={s.totalsWrap}>
          <View style={s.totalsBox}>
            <View style={s.tLine}>
              ...
              <Text style={s.tValue}>{fmt(data.sousTotal)}</Text>
            </View>
            {data.remiseTotale > 0 && (
              <View style={s.tLine}>
                <Text style={[s.tLabel, { color: ORANGE }]}>Remises</Text>
